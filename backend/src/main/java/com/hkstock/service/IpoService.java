...
public class IpoService {
  ...
  public Map<String, Object> getBreakRate() {
    ...
    long brokenCount =
        list.stream()
            .filter(
                ipo -> ipo.getFirstDayChange() != null && ipo.getFirstDayChange().doubleValue() < 0)
            .count();
  ...
  public Map<String, Object> getAiAnalysis(String stockCode) {
    ...
    try {
      Map response =
          restTemplate.getForObject(url, Map.class, stockCode, stockName, apiKey, baseUrl, model);
      if (response == null) {
