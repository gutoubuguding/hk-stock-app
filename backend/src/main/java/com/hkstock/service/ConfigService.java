 ...
 * 系统配置服务。
 *
 * <p>这个类专门负责读写 stock_config 表，并在内存中保留一份配置缓存。
 * 之前配置读写逻辑放在 ConfigController 里，业务服务如果想读取 AI 配置就要依赖 Controller，
 * 会让“接口层”和“业务层”耦合在一起。现在抽出来后：
 * <ul>
 *   <li>Controller 只负责接收/返回 HTTP 请求；</li>
 *   <li>Service 统一负责配置的读取、默认值和持久化；</li>
 *   <li>IpoService 等业务代码可以直接依赖 ConfigService，更清楚也更容易测试。</li>
 * </ul>
 */
@Service
public class ConfigService implements InitializingBean {
    ...
    /**
     * 简单内存缓存，避免每次请求都查数据库。
     * 当前项目是单机运行，HashMap 足够；如果以后多实例部署，再换成数据库直读或分布式缓存。
     */
    ...
    /**
     * Spring Bean 初始化后加载数据库配置。
     * 如果数据库还没建表或读取失败，也会填入默认值，保证设置页和 AI 分析接口可用。
     */
    @Override
    public void afterPropertiesSet() {
        configStore.clear();
        try {
            jdbc.query("SELECT config_key, config_value FROM stock_config", rs -> {
    ...
    /**
     * 更新并持久化 AI 模型配置。
     * body 来自设置页表单；api_key 允许为空字符串，因为用户可能想清空旧 Key。
     */
    ...
    /**
     * PostgreSQL upsert：有则更新，没有则插入。
     * 单个配置保存失败不影响应用继续运行，但会写日志方便排查。
     */
