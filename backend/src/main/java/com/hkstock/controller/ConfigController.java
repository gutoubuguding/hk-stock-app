 ...
 * 配置控制器。
 *
 * <p>只负责 HTTP 入参/出参：模型列表、当前配置、保存配置、连接测试。
 * 真正的数据库读写已经下沉到 {@link ConfigService}，避免业务层反向依赖 Controller。
 */
@RestController
...
public class ConfigController {
    ...
    public Map<String, Object> testFutuConnection(@RequestBody Map<String, Object> body) {
        ...
        try (Socket socket = new Socket()) {
            ...
            return Map.of("success", false, "message", "Futu OpenD 连接失败: " + e.getMessage(), "host", host, "port", port);
        }
    }
