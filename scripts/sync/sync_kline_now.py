#!/usr/bin/env python3
"""Sync K-line data for main stocks"""
import sys
...
for code in stocks:
    ...
    ret, data, _ = quote_ctx.request_history_kline(code, ktype=KLType.K_DAY, max_count=120)
    if ret != RET_OK:
        print(f'  Error: {data}')
        continue
    ...
    for _, row in data.iterrows():
        try:
            ...
            ''', (code, 'D', row['time_key'], row['open'], row['close'], row['high'], row['low'], row['volume'], row['turnover'], row.get('change_rate', 0), row.get('turnover_rate', 0)))
            count += 1
