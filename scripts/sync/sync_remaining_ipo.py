#!/usr/bin/env python3
"""Sync remaining IPO stocks K-line data with rate limiting"""
import sys
...
print("=" * 50)
print("补全剩余 IPO 股票 K 线数据")
print("=" * 50)

conn = psycopg2.connect(**DB_CONFIG)
cur = conn.cursor()

# Find IPO stocks without K-line data
cur.execute("""
    SELECT i.stock_code, i.stock_name, i.listing_date, i.issue_price
    FROM stock_ipo i
    LEFT JOIN (SELECT DISTINCT stock_code FROM stock_kline WHERE period_type = 'D') k 
        ON i.stock_code = k.stock_code
    WHERE i.listing_date >= '2025-01-01'
    AND k.stock_code IS NULL
    ORDER BY i.listing_date DESC
""")
stocks = cur.fetchall()
print(f"\n找到 {len(stocks)} 只没有 K 线数据的新股")
...
for i, (stock_code, stock_name, listing_date, issue_price) in enumerate(stocks, 1):
    futu_code = f"HK.{stock_code}"
    print(f"\n[{i}/{len(stocks)}] {stock_code} {stock_name}")
    ...
    # Calculate metrics if we have issue price and listing date
