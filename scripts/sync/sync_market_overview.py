    ...
    try:
        ...
        if df is not None and not df.empty:
            ...
            if pct_col is not None:
                # 转换为数值类型
                df[pct_col] = df[pct_col].astype(str).str.replace('%', '').str.replace('-', '0')
                df[pct_col] = df[pct_col].astype(float)
                
                raise_count = len(df[df[pct_col] > 0])
                fall_count = len(df[df[pct_col] < 0])
                equal_count = len(df[df[pct_col] == 0])
                
                print(f"全市场涨跌家数: 涨 {raise_count} / 跌 {fall_count} / 平 {equal_count}")
                return raise_count, fall_count, equal_count
            else:
                print(f"未找到涨跌幅列，可用列: {df.columns.tolist()}")
    except Exception as e:
        print(f"获取全市场涨跌家数失败: {e}")
...
def save_to_db(indices, breadth):
    ...
    for idx in indices:
        ...
        """, (
            ...
            idx["volume"], idx["turnover"], breadth[0], breadth[1], breadth[2], update_time
        ))
...
def main():
    ...
    # 2. 获取涨跌家数（优先用AKShare全市场数据，备用本地K线计算）
    breadth = fetch_market_breadth()
    if breadth is None:
        print("使用本地K线数据计算涨跌家数...")
        conn = psycopg2.connect(**DB_CONFIG)
        cur = conn.cursor()
    ...
    print(f"\n保存大盘概览 {len(indices)} 条，涨跌家数：涨 {breadth[0]} / 跌 {breadth[1]} / 平 {breadth[2]}")
    for idx in indices:
